...
console.log("[hostel] build v20260921B");

import { adminAuthContext } from "./auth.js?v=20260921B";
import { auth, updatePassword, reauthenticateWithCredential, EmailAuthProvider } from "./firebase-config.js?v=20260921B";
import { 
  branchService, 
  roomService, 
  studentService, 
...
} from "./services/db-service.js?v=20260921B";
...
function renderAllocationList() {
  const tbody = document.getElementById('allocation-list-tbody');
  const countEl = document.getElementById('allocation-list-count');
  if (!tbody) return;
  ...
  if (allotted.length === 0) {
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:20px;color:#94a3b8;">No students allotted to rooms yet.</td></tr>';
    return;
  }

  const sorted = [...allotted].sort((a, b) =>
...
function getFilteredRooms() {
  ...
  return (appState.rooms || []).filter(r => {
    // Fully occupied rooms are not selectable for allocation, so hide them entirely.
    const available = Math.max(0, (r.capacity || 4) - (r.occupied || 0));
...
function renderRoomGrid() {
  ...
  container.innerHTML = rooms.map(r => {
    ...
    const occupants = (r.studentIds || [])
      .map(stUid => (appState.students || []).find(s => s.uid === stUid || s.id === stUid))
      .filter(Boolean);
    const occupantsHtml = occupants.length > 0
      ? occupants.map(o => `<span class="room-occupant-chip" title="${escapeHtml(o.rollNo || '')}">${escapeHtml(o.name || o.rollNo || 'Student')}</span>`).join('')
...
async function confirmAllotment() {
  ...
  try {
    const queue = shuffleArray(roomIds.map(rId => {
      const r = (appState.rooms || []).find(x => x.id === rId);
...
function setupEvents() {
  ...
  const allotYearFilter = document.getElementById('allot-year-filter');
  if (allotYearFilter) allotYearFilter.addEventListener('change', (e) => { allotState.filterYear = e.target.value; renderStudentPanel(); });
